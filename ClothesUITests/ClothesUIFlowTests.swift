...
final class ClothesUIFlowTests: XCTestCase {
    ...
    func testLoginFormShowsValidationErrorWhenEmpty() throws {
        ...
        if app.buttons["已有账号登录"].waitForExistence(timeout: 3) {
        ...
        let inlineSubmit = app.buttons["sms.inline.submit.button"]
        XCTAssertTrue(inlineSubmit.waitForExistence(timeout: 8), "未显示内联短信登录提交按钮")
        XCTAssertFalse(inlineSubmit.isEnabled, "手机号与验证码为空时，登录按钮应禁用")
    }
    ...
    func testRegistrationFlowCanMoveForwardAndBackward() throws {
        ...
        guard app.buttons["注册"].waitForExistence(timeout: 3) else {
            throw XCTSkip("当前版本未登录态采用内联短信登录，不再展示独立注册流程入口")
        }
        app.buttons["注册"].tap()
