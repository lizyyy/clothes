...
final class ClothesUITests: XCTestCase {
  ...
  private func ensureLoggedOut(_ app: XCUIApplication) {
    ...
    if app.buttons["profile.logout.button"].waitForExistence(timeout: 3) {
      ...
      XCTAssertTrue(app.buttons["profile.register.button"].waitForExistence(timeout: 5), "退出登录后应显示注册按钮")
    }
  }
  ...
  func testUserRegistration() throws {
    ...
    XCTAssertTrue(registerButton.waitForExistence(timeout: 10), "设置页里没看到『注册』按钮")
    registerButton.tap()
  ...
  func testExistingAccountLogin() throws {
    ...
    if app.buttons["profile.logout.button"].waitForExistence(timeout: 3) {
      ...
      app.buttons["profile.logout.button"].tap()
      XCTAssertTrue(app.buttons["profile.login.button"].waitForExistence(timeout: 5), "退出后应显示登录按钮")
    }
    ...
    // Fill in credentials using accessibility identifiers
  ...
  func testSwitchAccount() throws {
    ...
    if !app.buttons["profile.logout.button"].waitForExistence(timeout: 3) {
      ...
      usernameField.tap()
      usernameField.typeText(firstAccount.username)

      let passwordField = app.secureTextFields["login.password.field"]
      XCTAssertTrue(passwordField.waitForExistence(timeout: 10), "应显示密码输入框")
      passwordField.tap()
      passwordField.typeText(firstAccount.password)

      dismissKeyboardIfNeeded(app)

      let confirmLoginButton = app.buttons["login.submit.button"]
      XCTAssertTrue(confirmLoginButton.waitForExistence(timeout: 10), "应显示登录按钮")
      confirmLoginButton.tap()
      declineSavePasswordPromptIfNeeded(app)
    ...
    let loginButton2 = app.buttons["profile.login.button"]
    if loginButton2.waitForExistence(timeout: 5) {
      loginButton2.tap()

      let usernameField2 = app.textFields["login.username.field"]
      XCTAssertTrue(usernameField2.waitForExistence(timeout: 10), "应显示用户名输入框")
      usernameField2.tap()
      usernameField2.typeText(secondAccount.username)

      let passwordField2 = app.secureTextFields["login.password.field"]
      XCTAssertTrue(passwordField2.waitForExistence(timeout: 10), "应显示密码输入框")
      passwordField2.tap()
      passwordField2.typeText(secondAccount.password)
